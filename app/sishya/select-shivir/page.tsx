...
export default function SishyaSelectShivirPage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      // Get ALL Shivirs this Sishya is assigned to
      ...
      // If only one — auto select and go to dashboard
      if (myShivirIds.length === 1) {
        localStorage.setItem('sishyaSelectedShivirId', myShivirIds[0]);
        window.location.href = '/sishya';
        return;
      }

      // Multiple — load details
  ...
  const selectShivir = (shivirId: string) => {
    ...
    localStorage.setItem('sishyaSelectedShivirId', shivirId);
    window.location.href = '/sishya';
  };
    ...
    <div className="min-h-screen bg-orange-50 p-4">
      <div className="max-w-lg mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow p-5 mb-6 text-center">
          ...
          <p className="text-gray-500 text-sm mt-1">Select the Shivir you want to view</p>
        </div>
